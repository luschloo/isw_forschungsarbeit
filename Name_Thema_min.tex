...
\begin{document} 
	...
	\begin{longtable}{p{0.5cm}p{10.5cm}p{2cm}}
		...
		11 & Visualierung der Daten & Kann  \\
	...
	\begin{figure}
		...
		\begin{tikzpicture}[node distance=2cm]
			...
			\node[draw, rectangle, align=center, fill=green!20] (box2) at (3cm,0) {Anlagenvisualisierung\\(spätere Integration in roboTools)};
		...
		\begin{tikzpicture}
			...
			\begin{umlpackage}{Model}
				\umlclass[name=com, x=0, y=-6]{Communicator}{
					...
					+ Sende Aktualisierungs-Telegramm \\Schwere Systemereignis(): void\\
					+ Sende Positionsdaten(): void
				}
				
				\umlclass[name=robot, x=8, y=-4]{Robot}{
					- Temperatur: int \\
					- Fehler: bool \\
				...
				\umlclass[name=interpreter, x=7, y=-8]{Interpreter}{
					- Nachricht: byte[]
				}{
			...
			\umlassoc[pos=0.5]{Communicator}{Interpreter}
			\umlassoc[pos=0.5]{Communicator}{Robot}
			\umlassoc[pos=0.5]{Interpreter}{Robot}
	...
	Anschließend werden die gewandelten Bytes in den Byte-Array des zusendeten Datentelegramms geschrieben. Das fertige Datentelegramm wird anschließend über folgenden Befehl versendet:
	\begin{lstlisting}
	...
	Handelt es sich anstelle des Datenanforderungstelegramm um ein Telegramm zur Aktualisierung der zu übertragenden Systemereignis-Schwere, wird eine globale Variable, welche die zu übertragende Schwere des Roboters repräsentiert überschrieben.\\
	\textbf{Systemereignisse}\\
	Um die Systemereignisse über TCP/IP an die Anlagenvisualisierung übertragen zu können müssen diese erst ausgelesen werden. Hierzu gibt es in VAL~3 zwei Möglichkeiten. Die eine Funktion ermöglicht die Ausgabe des gesamten Systemereignisses in ausformulierter Textform, die andere wiederum ermittelt nur den Ereigniscode. Diese beiden Funktionen werden nachfolgend in einer Vergleichstabelle mit Hilfe von Harvey Balls verglichen (vgl. Tabelle \ref{tab:getEvent-vergleich}). Der Füllheitsgrad des Harvey Balls entspricht dabei dem Grad der Zielerreichung.
	\begin{table}[h]
		\centering
	...
	\end{table}\newline
	Für den Einsatz des Ereigniscodes spricht, dass der Umfang der zu übertragenden Daten viel geringer ist. Dafür muss bei dem Empfänger, der Anlagenvisualisierung, die Bedeutung der jeweiligen Ereigniscodes hinterlegt werden.
	\section{WPF-Anwendung}
